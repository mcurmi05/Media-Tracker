// Per-user watch status for TV shows, stored once per (user, media_entries)
...
// Upsert the full status object for a title (keyed on user_id + entry_id).
export async function saveWatchStatus(userId, movieEntryId, status) {
  if (!userId || !movieEntryId) return;
  const { error } = await supabase.from("user_watch_status").upsert(
    {
      user_id: userId,
